        ...
        if (spellInfo)
        {
            ...
            if(spellRangeEntry)
            {
                ...
                if (spellRangeEntry->ID == 1 && spellInfo->EffectRadiusIndex[EFFECT_INDEX_0])
                {
                    ...
                    if (spellRadiusEntry)
                        spellMaxRange = spellRadiusEntry->Radius > ATTACK_DISTANCE ? spellRadiusEntry->Radius - AOE_LEEWAY : spellRadiusEntry->Radius;
                }
...
bool PlayerbotAI::CanCastSpell(uint32 spellid, Unit* target, uint8 effectMask, bool checkHasSpell, Item* itemTarget, bool ignoreRange, bool ignoreInCombat, bool ignoreMount, SpellCastResult* checkResult)
{
    ...
    if (bot->hasUnitState(UNIT_STAT_CAN_NOT_REACT_OR_LOST_CONTROL))
    {
        ...
        const std::list<uint32> ignoreOutOfControllSpells = { 642, 1020, 1499, 1953, 7744, 11958, 13795, 13809, 13813, 14302, 14303, 14304, 14305, 14310, 14311, 14316, 14317, 22812, 27023, 27025, 34600, 42292, 47585, 49039, 49055, 49056, 49066, 49067, 50334, 51490, 59156, 59158, 59159, 59752, 63148, 65547 };
	...
	if (!itemTarget)
	{
        ...
        const std::list<uint32> neutralSpells = { 1499, 5384, 13795, 13809, 13813, 14302, 14303, 14304, 14305, 14310, 14311, 14316, 14317, 27023, 27025, 34600, 49055, 49056, 49066, 49067, 51490, 59156, 59158, 59159 };
