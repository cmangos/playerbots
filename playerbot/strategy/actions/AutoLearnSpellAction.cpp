...
#endif
#if defined(MANGOSBOT_ONE) || defined(MANGOSBOT_TWO) // TBC OR WOTLK
			if (IgnoreSpellsThatDoNotBelong(tSpell->spell, bot->getClass()))
			{
...
bool AutoLearnSpellAction::LearnSpellFromSpell(uint32 spellId, std::ostringstream* out)
{
	...
	for (int j = 0; j < 3; ++j)
	{
		if (proto->Effect[j] == SPELL_EFFECT_LEARN_SPELL)
		{
			...
			if (IgnoreSpellsThatDoNotBelong(learnedSpell, 0))
			{
...
/**
* Some spells are being learned when they shouldn't be, either they are left over spells from previous patches or from mobs, or just unused blizz spells. 
*/
bool AutoLearnSpellAction::IgnoreSpellsThatDoNotBelong(uint32 spellId, uint8 classId)
{
	return
