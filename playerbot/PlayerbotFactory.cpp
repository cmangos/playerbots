...
bool PlayerbotFactory::CanEquipArmor(ItemPrototype const* proto)
{
    ...
    for (int j = 0; j < MAX_ITEM_PROTO_STATS; ++j)
    {
        ...
        AddItemStats(proto->ItemStat[j].ItemStatType, proto->ItemStat[j].ItemStatValue, sp, ap, tank, heal);
    }
...
bool PlayerbotFactory::CheckItemStats(uint16 sp, uint16 ap, uint16 tank, uint16 heal, uint16 minValue, uint8 specTab)
{
    ...
    bool isApItem = ap >= minValue && ap >= sp && ap >= tank && ap >= heal;
    ...
    case CLASS_DRUID:
        if (botSpecTab == 0)
            return isSpItem;
        else if (botSpecTab == 2)
            return isHealerItem;
        else if (botSpecTab == 1)
            return isApItem;
        break;
    case CLASS_HUNTER:
        return isApItem;
...
void PlayerbotFactory::AddItemStats(uint32 mod, uint32 amount, uint16 &sp, uint16 &ap, uint16 &tank, uint16 &heal)
{
    switch (mod)
    {
    case ITEM_MOD_MANA: // *1
        sp += amount;
        heal += amount;
        break;
    case ITEM_MOD_HEALTH: // *1
        sp += amount;
        ap += amount;
        tank += amount * 2;
        heal += amount;
        break;
    case ITEM_MOD_AGILITY: // *15
        ap += amount * 15;
        tank += amount * 15;
        break;
    case ITEM_MOD_STRENGTH: // *15
        ap += amount * 15;
        tank += amount * 10;
        break;
    case ITEM_MOD_INTELLECT: // *15
        sp += amount * 15;
        heal += amount * 15;
        break;
    case ITEM_MOD_SPIRIT: // *15
        sp += amount * 15;
        heal += amount * 15;
        break;
    case ITEM_MOD_STAMINA: // *10
        sp += amount * 10;
        ap += amount * 10;
        tank += amount * 15;
        heal += amount * 10;
    ...
    case ITEM_MOD_EXPERTISE_RATING: // *15
        tank += amount * 15;
        break;
    ...
    case ITEM_MOD_HASTE_RANGED_RATING: // *5
        ap += amount * 5;
        break;
    case ITEM_MOD_HIT_SPELL_RATING:
    case ITEM_MOD_CRIT_SPELL_RATING:
    case ITEM_MOD_HASTE_SPELL_RATING: // *5
        sp += amount * 5;
        break;
    ...
    case ITEM_MOD_CRIT_TAKEN_RATING: // *5
        tank += amount * 5;
        break;
    case ITEM_MOD_HIT_RATING: // *15
        sp += amount * 15;
        ap += amount * 15;
        tank += amount * 15;
        heal += amount * 15;
        break;
    case ITEM_MOD_CRIT_RATING: // *15
        sp += amount * 15;
        ap += amount * 15;
        heal += amount * 10;
        break;
    case ITEM_MOD_RESILIENCE_RATING: // *15
        sp += amount * 15;
        ap += amount * 15;
        tank += amount * 15;
        heal += amount * 15;
        break;
    case ITEM_MOD_HASTE_RATING: // *15
        sp += amount * 15;
        ap += amount * 10;
        heal += amount * 15;
        break;
    case ITEM_MOD_ATTACK_POWER: // *7
        ap += amount * 7;
        break;
    case ITEM_MOD_MANA_REGENERATION: // *6
        heal += amount * 30;
        break;
    case ITEM_MOD_ARMOR_PENETRATION_RATING: // *15
        ap += amount * 15;
        break;
    case ITEM_MOD_SPELL_POWER: // *13
        sp += amount * 13;
        heal += amount * 13;
        break;
    case ITEM_MOD_SPELL_PENETRATION: // *12
        sp += amount * 12;
        break;
    case ITEM_MOD_BLOCK_VALUE:// *3
        tank += amount * 3;
        break;
...
void PlayerbotFactory::AddItemSpellStats(uint32 smod, uint16& sp, uint16& ap, uint16& tank)
{
    switch (smod)
    {
...
void PlayerbotFactory::InitEquipment(bool incremental, bool syncWithMaster, bool progressive, bool partialUpgrade)
{
    ...
    std::vector<bool> upgradeSlots(EQUIPMENT_SLOT_END, false);
    if (incremental && partialUpgrade)
    {
    ...
    if (syncWithMaster && ai->GetMaster())
    {
        ...
        if (botLevel != searchLevel)
        {
            searchLevel = botLevel;
        }
    }
    ...
    for (uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        */
        ...
        if (incremental && partialUpgrade && (slot >= upgradeSlots.size() || !upgradeSlots[slot]) && !(slot == EQUIPMENT_SLOT_TRINKET1 || slot == EQUIPMENT_SLOT_TRINKET2))
            continue;
        ...
        else if (progressiveGear)
        {
            ...
            if (!incremental && urand(0, 100) < 100 * sPlayerbotAIConfig.randomGearLoweringChance && quality > ITEM_QUALITY_NORMAL)
            {
                quality--;
            }
        }
        ...
        do
        {
            // Check if the slot is for a shirt or tabard
            if (slot == EQUIPMENT_SLOT_BODY || slot == EQUIPMENT_SLOT_TABARD)
            {
                // Retrieve items matching the bot's level, quality, and slot
                std::vector<uint32> ids = sRandomItemMgr.Query(60, 1, 1, slot, 1);
                if (!ids.empty())
                {
                    Shuffle(ids); // Shuffle only if items are available

                    sLog.outDetail("Bot #%d %s:%d <%s>: %u possible items for slot %d",
                        bot->GetGUIDLow(),
                    ...
                    {
                        // Get item prototype only once per iteration
                        ItemPrototype const* proto = sObjectMgr.GetItemPrototype(newItemId);
                        if (!proto || proto->Quality > ITEM_QUALITY_UNCOMMON || proto->ItemLevel > maxItemLevel)
                            continue;

                        // Check unique or max count restrictions once
                        ...
                        // Skip equipping if the bot already has the item
                        Item* oldItem = bot->GetItemByPos(INVENTORY_SLOT_BAG_0, slot);
                        if (oldItem && oldItem->GetProto()->ItemId == newItemId)
                            continue;

                        // Determine if the item can be equipped
                        uint16 eDest;
                        if (RandomPlayerbotMgr::CanEquipUnseenItem(bot, slot, eDest, newItemId) == EQUIP_ERR_OK)
                        {
                            // Destroy old item if present and equip the new one
                            if (oldItem)
                            ...
                            {
                                found = true;
                                break;  // Exit loop after equipping an item
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                for (uint32 q = quality; q < ITEM_QUALITY_ARTIFACT; ++q)
                {
                    ...
                    // Additional queries for specific cases (e.g., tanks, casters)
                    if ((slot == EQUIPMENT_SLOT_MAINHAND) &&
                        ((specId == 3 || specId == 5) ||  // Tank specs
                            (specId == 4 || ((bot->getClass() == CLASS_DRUID && (specId == 29 || specId == 31)) || bot->getClass() == CLASS_PRIEST ||
                                bot->getClass() == CLASS_MAGE || bot->getClass() == CLASS_WARLOCK || (specId == 20 || specId == 22))) ||  // Caster specs
#ifdef MANGOSBOT_ZERO
                                (bot->getClass() == CLASS_ROGUE || specId == 2)))  // Dual wield (MANGOSBOT_ZERO case)
#else
                        (bot->getClass() == CLASS_ROGUE || specId == 2 || specId == 21 || specId == 17 || specId == 18 || specId == 19)))  // Dual wield (other case)
#endif
                    {
                        std::vector<uint32> oneHanded = sRandomItemMgr.Query(level, bot->getClass(), uint8(specId), EQUIPMENT_SLOT_OFFHAND, q);
                        ids.insert(ids.end(), oneHanded.begin(), oneHanded.end());  // Insert at the end to avoid reordering
                    }
                }
                ...
                if (incremental || !progressiveGear)
                {
                    ...
                    std::sort(ids.begin(), ids.end(), [specId](int a, int b)
                        {
                            // Calculate base comparison value for items a and b
                            uint32 baseCompareA = sRandomItemMgr.GetStatWeight(a, specId) + sRandomItemMgr.GetBestRandomEnchantStatWeight(a, specId);
                            uint32 baseCompareB = sRandomItemMgr.GetStatWeight(b, specId) + sRandomItemMgr.GetBestRandomEnchantStatWeight(b, specId);

                            // Retrieve item prototypes only if needed
                            if (baseCompareA != baseCompareB)
                                return baseCompareA < baseCompareB;

                            ItemPrototype const* protoA = sObjectMgr.GetItemPrototype(a);
                            ItemPrototype const* protoB = sObjectMgr.GetItemPrototype(b);

                            // Secondary comparison by item quality and item level
                            return (protoA->Quality * protoA->ItemLevel) < (protoB->Quality * protoB->ItemLevel);
                        });
                ...
                std::unordered_set<uint32> lockedItemsSet(lockedItems.begin(), lockedItems.end());
                std::unordered_set<uint32> unavailableItemsSet(unavailableItemIDs.begin(), unavailableItemIDs.end());
                std::unordered_set<uint32> blacklistSet(sPlayerbotAIConfig.randomGearBlacklist.begin(), sPlayerbotAIConfig.randomGearBlacklist.end());

                // Init constants
                const bool isCasterClass = bot->getClass() == CLASS_PRIEST || bot->getClass() == CLASS_MAGE ||
                    bot->getClass() == CLASS_WARLOCK || (bot->getClass() == CLASS_DRUID && (specId == 29 || specId == 31));
                ...
                for (uint32 index = 0; index < ids.size(); ++index)
                {
                    ...
                    // Skip unique equippable items if already owned
                    if ((proto->Flags & ITEM_FLAG_UNIQUE_EQUIPPABLE && bot->HasItemCount(proto->ItemId, 1)) ||
                        (proto->MaxCount && bot->HasItemCount(proto->ItemId, proto->MaxCount)))
                        continue;
                    ...
                    if ((slot == EQUIPMENT_SLOT_OFFHAND && (specId == 3 || specId == 5) && !(proto->Class == ITEM_CLASS_ARMOR && proto->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD)) ||
                        ...
                        if (weaponType)
                        {
                            if (slot == EQUIPMENT_SLOT_MAINHAND)
                            {
                                if ((proto->Class == ITEM_CLASS_ARMOR && proto->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD) ||
                                    (weaponType == INVTYPE_2HWEAPON && proto->Class == ITEM_CLASS_WEAPON && proto->InventoryType != INVTYPE_2HWEAPON) ||
                                    (weaponType != INVTYPE_2HWEAPON && proto->Class == ITEM_CLASS_WEAPON && proto->InventoryType == INVTYPE_2HWEAPON) ||
                            ...
                            {
                                // If weapon type is not 2H and class is restricted, skip if not a holdable item
                                if (weaponType != INVTYPE_2HWEAPON && isCasterClass && proto->InventoryType != INVTYPE_HOLDABLE)
                                    continue;

                                // If weapon is 2H and class is restricted, skip
                                if (weaponType == INVTYPE_2HWEAPON && isCasterClass)
                                    continue;

                                // Skip if it's a 2H weapon but the item is not a 2H weapon
                        ...
                        // Random enchant calculation
                        uint32 randomEnchBestId = 0, randomEnchBestValue = 0;
                        ...
                        // Handling offhand and mainhand weapon stats
#ifdef MANGOSBOT_ZERO
                        bool isRogue = (bot->getClass() == CLASS_ROGUE || specId == 2);
#endif
#ifdef MANGOSBOT_ONE
                        bool isRogue = (bot->getClass() == CLASS_ROGUE || specId == 2 || specId == 21);
#endif
#ifdef MANGOSBOT_TWO
                        bool isRogue = (bot->getClass() == CLASS_ROGUE || specId == 2 || specId == 21 || specId == 17 || specId == 18 || specId == 19);
#endif

                        // Check for weapon and offhand slot once
                        if (proto->IsWeapon() && slot == EQUIPMENT_SLOT_OFFHAND && isRogue)
                        {
                            bool betterValue = false, betterDamage = false, betterDps = false;
                            ...
                            if (mhItem && mhItem->GetProto())
                            {
                                ...
                                uint32 ohDps = 0, ohDamage = 0;

                                // Calculate mainhand DPS and damage
                                for (int i = 0; i < MAX_ITEM_PROTO_DAMAGES; i++)
                                {
                                    if (mhProto->Damage[i].DamageMax == 0) break;
                                ...
                                // Calculate offhand DPS and damage
                                for (int i = 0; i < MAX_ITEM_PROTO_DAMAGES; i++)
                                {
                                    if (proto->Damage[i].DamageMax == 0) break;
                                    ohDamage = proto->Damage[i].DamageMax;
                                    ohDps = (proto->Damage[i].DamageMin + proto->Damage[i].DamageMax) / (float)(proto->Delay / 1000.0f) / 2;
                                }

                                // Compare DPS and damage values
                                betterDps = (ohDps > mhDps);
                                betterDamage = (ohDamage > mhDamage);
                            }
                        ...
                        if (found)
                        {
                            ...
                            {
                                if (oldItem)
                                    sLog.outDetail("Bot #%d %s:%d <%s>: Old Item: slot: %u, id: %u, value: %u (%s)", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", botLevel, bot->GetName(), slot, oldProto->ItemId, oldStatValue, oldProto->Name1);
                                sLog.outDetail("Bot #%d %s:%d <%s>: New Item: slot: %u, id: %u, value: %u (%s)", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", botLevel, bot->GetName(), slot, proto->ItemId, newStatValue, proto->Name1);
                            }
                            break;
                        }
                }
        ...
        if (!found)
        {
            if (slot != EQUIPMENT_SLOT_TRINKET1 && slot != EQUIPMENT_SLOT_TRINKET2)
                sLog.outDetail("Bot #%d %s:%d <%s>: no items for slot %d, quality >= %u", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", botLevel, bot->GetName(), slot, quality);
            continue;
        }
    }

    /*if (incremental && oldGS != newGS)
        sLog.outBasic("Bot #%d %s:%d <%s>: GS: %u -> %u", bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H", botLevel, bot->GetName(), oldGS, newGS);*/
                            ...
                            if (SpellItemEnchantmentEntry const* pEnchant = sSpellItemEnchantmentStore.LookupEntry(gemProperty->spellitemenchantement))
                            {
                                ...
                                if (GemColor & SocketColor)
                                {
                                    for (int i = 0; i < 3; ++i)
                                    {
                                        ...
                                        AddItemStats(pEnchant->spellid[i], pEnchant->amount[i], sp, ap, tank, heal);
                                    }
                                }

                                if (!CheckItemStats(sp, ap, tank, heal))
                                    continue;
...
void PlayerbotFactory::InitGems()
{
#ifndef MANGOSBOT_ZERO
    std::vector<uint32> gems = sRandomItemMgr.GetGemsList();
               ...
               /*bool hasSockets = false;
                ...
                for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT; enchant_slot < SOCK_ENCHANTMENT_SLOT + MAX_GEM_SOCKETS; ++enchant_slot)
                {
                    ...
                    case SOCKET_COLOR_META:
                        ...
                        case CLASS_DEATH_KNIGHT: 
                            if (botSpecTab == 0)
                                gem_id = 41380; // Austere Earthsiege Diamond
                            else 
                                gem_id = 41398; // Relentless Earthsiege Diamond
                            break;
#endif
                        case CLASS_DRUID: 
                            if (botSpecTab == 0)
                                gem_id = 41285; // Chaotic Skyflare Diamond
                            else if (botSpecTab == 2)
                                gem_id = 41401; // Insightful Earthsiege Diamond
                            else if (botSpecTab == 1)
                                gem_id = 41398; // Relentless Earthsiege Diamond
                            break;
                        case CLASS_HUNTER: 
                        ...
                        case CLASS_WARRIOR: 
                            if (botSpecTab == 2)
                                ...
                                gem_id = 41398; // Relentless Earthsiege Diamond
                            break;
                        default: gem_id = 41398; // Relentless Earthsiege Diamond
                            break;
                        }
                        break;
                    default:
                        ...
                        for (std::vector<uint32>::const_iterator itr = gems.begin(); itr != gems.end(); itr++)
                        {
                            if (ItemPrototype const* gemProto = sObjectMgr.GetItemPrototype(*itr))
                            {
                                ...
                                if (GemPropertiesEntry const* gemProperty = sGemPropertiesStore.LookupEntry(gemProto->GemProperties))
                                {
                                    ...
                                    {
                                        uint32 GemColor = gemProperty->color;
                                        uint32 GemStatsAmount = 0;
                                        for (int i = 0; i < 3; i++)
                                        {
                                            if (pEnchant->type[i] == ITEM_ENCHANTMENT_TYPE_STAT)
                                                GemStatsAmount += pEnchant->amount[i];
                                        }
                                        ...
                                        if (GemColor & SocketColor)
                                        {
                                            for (int i = 0; i < 3; ++i)
                                            {
                                                ...
                                                // skip if low gem but good item
                                                if ((proto->ItemLevel) >= 200 && GemStatsAmount < 20 && pEnchant->spellid[i] != 68251)
                                                    continue;
